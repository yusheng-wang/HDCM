...
Da <- rbind(HDCM2_fit, Real_PM25, CMAQ_PM25) %>%
    filter(month(DATE_TIME) %in% 6,
           day(DATE_TIME) %in% c(19:24))
...
p <- ggplot(Da, aes(x = DATE_TIME, group  = Model)) +
  ...
  scale_x_continuous(
    # expand = c(1e-5, 0),
                     ...
                     , labels = c("Jun 19, 2015", "Jun 20, 2015",
                                  "Jun 21, 2015", "Jun 22, 2015",
...
p
ggsave(plot = p, paste0("./figure/", 'Fig7_', City_Name, "_S", ".pdf"),
