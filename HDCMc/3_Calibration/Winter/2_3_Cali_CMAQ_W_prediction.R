...
Da$Date <- paste0("Dec", " ",day(Da$DATE_TIME), ", ", "2015")
PM2.5$Date <- paste0("Dec", " ", day(PM2.5$DATE_TIME), ", ", "2015")
...
ggsave(plot = p1, paste0(paste0("./figure/"), 'Fig7_Mean',
...
int <- 1e0
{
  ...
  CMAQ_Cali_PM25$Date <- paste0("Dec", " ", day(CMAQ_Cali_PM25$DATE_TIME), 
                                ", ",  "2015")
  ...
  p1 <- ggplot() +
    ...
    #            )
    #            , size = 0.6, pch = 18) +
    facet_grid(Method ~ Date, labeller = labeller(Method = Label), switch = "both") +
    scale_fill_distiller(palette = "Spectral", 
                         name = expression(atop(atop(textstyle("PM"[2.5]),
                                                     textstyle(paste("(", mu, g, "/", m^{3},")"))), NA)), 
                         limits = c(y.min,  y.max),
                         breaks = c(round(seq(y.min, y.max, int), 0)),
                         labels = c(round(seq(y.min, y.max, int), 0))
    ) +
    # scale_colour_distiller(palette = "Spectral", 
    #                        name = "PM25", 
    #                        limits=c(0, 700),
...
ggsave(plot = p1, paste0(paste0("./figure/"), 'FigS4_W_Mean_Wts',".pdf"),
       dpi= 500, width  = 9, height = 2.5)
