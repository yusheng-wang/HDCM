...
###################################################################
# in summer
HDCM <- Tab[grepl("Fit_HDCM2_S", Tab)]
load(paste0(file, "data/",HDCM))
load(paste0(file, "data/", "train_HDCM2_S.RData"))
y.fit <- test.HDCM(test = train, Ks = Re$Ks, PIU = Re$PIU, seed = 1234)
y.fit <- (apply(y.fit, c(1, 2), quantile, prob = 0.5))
HDCM2_Residuals_S <- as.vector(y.fit - train$Y_ts)
range(HDCM2_Residuals_S)
# in winter
...
# pdf(file = "./figure/Residuals_S.pdf", width = 18, height = 7)
{
  p1 <- ggplot(data = da0, aes(x = x, y = density)) + 
    ...
    geom_text(aes(x = -500#min(da0$x)
                  , y = 0.05#max(da0$density) 
                  , label =  "(a)"),
              # family = c("sans"),
              # fontface = 'bold',
              color = "black", size = 10) +
                                             ...
                                             "After calibration"))
{
  p2 <- ggplot(da, aes(x = index, y = error)) +
    ...
    theme_bw()+ 
    ylim(x_range[1], x_range[2]) +  
    geom_text(aes(x = min(da$index), y = 400##max(da$error) 
                                , label =  "(b)"),
                            # family = c("sans"),
                            # fontface = 'bold',
                            color = "black", size = 10) + 
...
##############################################################################
ggsave(plot = p, file = "./figure/Fig8_Residuals_S.png", #Residuals_S.pdf
...
# pdf(file = "./figure/Residuals_W.pdf", width = 18, height = 7)
{
  p3 <- ggplot(data = da0, aes(x = x, y = density)) + 
    # geom_point(aes(shape = method), size = 5) +
    geom_line(aes(linetype = method, col = method), size = 1.5) +
    # scale_shape_manual(name = '', values =  c('*', '+'), labels = label) + 
    scale_linetype_manual(name = '', values=  c("dashed", "solid"),
...
da$method <- ordered(da$method, levels = c("Before calibration",
                                           "After calibration"))
{
  p4 <- ggplot(da, aes(x = index, y = error)) +
    geom_point(aes(shape = method, col = method), size = 4) +
    
    scale_color_manual(name = '', values =  c("black", "blue"), labels = label)+
    theme_bw() + ylim(x_range[1], x_range[2]) +  
    geom_text(aes(x = 1, y = max(da$error) 
                  , label =  "(d)"),
              # family = c("sans"),
              # fontface = 'bold',
              color = "black", size = 10) +
...
# p
##############################################################################
# ggsave(plot = p, file = "./figure/FigureS2_Residuals_W.png", #Residuals_S.pdf
#        width = 18, height = 7, dpi = 300)
##############################################################################
# ##############################################################################
# ggsave(plot = p, file = "./figure/FigureS2_Residuals.png", #Residuals_S.pdf
